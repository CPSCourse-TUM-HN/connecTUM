...
class Position {
  ...
  */
  ...
  /**
   * Constructor that initializes the board from a 2D array
   * @param board: 2D array where:
   *              1 represents current player's pieces
   *              -1 represents opponent's pieces
   *              0 represents empty cells
   * @param current_player: 1 if it's the first player's turn, -1 if it's the second player's turn
   */
  Position(const int board[HEIGHT][WIDTH], int current_player = 1) : current_position{0}, mask{0}, moves{0} {
    // Convert the 2D array to bitboard representation
    for(int col = 0; col < WIDTH; col++) {
      for(int row = 0; row < HEIGHT; row++) {
        if(board[row][col] != 0) {
          position_t pos = UINT64_C(1) << (row + col * (HEIGHT + 1));
          mask |= pos;  // Mark this position as occupied
          if(board[row][col] == current_player) {
            current_position |= pos;  // Mark this position as current player's piece
          }
          moves++;
        }
      }
 ...
 public:
   ...
   *         0 represents empty cells
   */
  std::array<std::array<int, WIDTH>, HEIGHT> get_board() const {
    ...
    for(int col = 0; col < WIDTH; col++) {
      for(int row = 0; row < HEIGHT; row++) {
        position_t pos = UINT64_C(1) << (row + col * (HEIGHT + 1));
        if(mask & pos) {  // If there's a piece at this position
          if(current_position & pos)
            board[HEIGHT - 1 - row][col] = 1;    // Current player's piece
          else
            board[HEIGHT - 1 - row][col] = -1;   // Opponent's piece
        }
      }
    }
  ...
  /**
   * Draw the current position to stdout
   * Uses 'X' for current player, 'O' for opponent, and '.' for empty cells
   */
  void draw() const {
    ...
    for(int col = 0; col < WIDTH; col++) {
      for(int row = HEIGHT - 1; row >= 0; row--) {
        ...
        if(mask & pos) {  // If there's a piece at this position
          if(current_position & pos)
            board[row][col] = 'X';  // Current player's piece
          else
            board[row][col] = 'O';  // Opponent's piece
        }
      }
    }
